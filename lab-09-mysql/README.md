...
3. В `functions.php` подключите `db.php` и перепишите:
   ...
   ```
4. Заполните таблицу несколькими своими записями (`INSERT`), запустите `php -S localhost:8000 -t .`
   (из папки этой лабы) и убедитесь, что записи из БД появились на странице.
...
`https://github.com/JLMoodle/diary-blog-template/tree/students/lab-09-mysql/<Фамилия>`.
