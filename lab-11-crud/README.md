...
В этой папке `lab-11-crud/` вы создаёте свою подпапку по фамилии и ведёте код только в ней.

1. Перед началом работы забрать чужие правки: `git pull --rebase`.
2. Создать свою подпапку: `mkdir lab-11-crud/<Фамилия>` (латиницей, например `kimsan`).
...
```bash
git add lab-11-crud/<Фамилия>/
git commit -m "[lab-11] <Фамилия>: <что сделано>"
git pull --rebase
git push
```

Отправьте преподавателю одну строку: `Сдаю: lab-11` + ссылку на вашу папку
`https://github.com/JLMoodle/diary-blog-template/tree/students/lab-11-crud/<Фамилия>`.
